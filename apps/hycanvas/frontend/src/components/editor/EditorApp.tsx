...
export function EditorApp() {
  ...
  const returnUrlParam = Array.isArray(router.query.returnUrl) ? router.query.returnUrl[0] : router.query.returnUrl;
  const returnLabelParam = Array.isArray(router.query.returnLabel) ? router.query.returnLabel[0] : router.query.returnLabel;
  let integrationReturnUrl: string | null = null;
  if (returnUrlParam) {
    try {
      const parsed = new URL(returnUrlParam);
      const configuredOrigin = process.env.NEXT_PUBLIC_CONTENTFLOW_URL;
  ...
  const integrationReturnLabel = returnLabelParam?.trim().slice(0, 40) || tr("editor.back_to_dashboard");
  const returnToIntegration = useCallback(() => {
    if (!integrationReturnUrl) return;
    if (window.parent !== window) {
      window.parent.postMessage({ type: "hycanvas:return", returnUrl: integrationReturnUrl }, new URL(integrationReturnUrl).origin);
      return;
    }
  ...
  return (
    <div className="flex h-screen w-screen flex-col overflow-hidden bg-neutral-100 text-neutral-900">
      ...
      <header className="flex flex-nowrap items-center gap-3 border-b border-neutral-200 bg-surface px-3 py-2">
        ...
        <IconButton className="shrink-0" aria-label={integrationReturnUrl ? integrationReturnLabel : tr("editor.back_to_dashboard")} onClick={() => integrationReturnUrl ? returnToIntegration() : void router.push("/dashboard")}>
        ...
        <div className="ms-auto flex shrink-0 items-center gap-1">
          ...
          {integrationReturnUrl && (
            <Button size="sm" onClick={() => void saveAndReturn()} disabled={!designId || saving || accessMode !== "edit"}>
              {saving ? tr("editor.saving") : `${tr("editor.save")} · ${integrationReturnLabel}`}
            </Button>
          )}
        </div>
      </header>
