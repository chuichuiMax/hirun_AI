...
import { isContentSwarmManaged, requestContentSwarmMaterials, type ContentSwarmGallery, type ContentSwarmMaterial } from "@/lib/managedAuth";
...
export function UploadsPanel({ workspaceId }: { workspaceId: string | null }) {
  ...
  const [managedGalleries, setManagedGalleries] = useState<ContentSwarmGallery[]>([]);
  const [managedCategoryId, setManagedCategoryId] = useState("");
  const [managedRefreshNonce, setManagedRefreshNonce] = useState(0);
  const [query, setQuery] = useState("");
  ...
  const refreshManagedGalleries = useCallback(async () => {
    if (!isContentSwarmManaged) return;
    const result = await requestContentSwarmMaterials<{ galleries: ContentSwarmGallery[] }>("list-galleries");
    setManagedGalleries(result.galleries ?? []);
  }, []);

  useEffect(() => {
    if (!isContentSwarmManaged) return;
    let cancelled = false;
    void requestContentSwarmMaterials<{ galleries: ContentSwarmGallery[] }>("list-galleries")
      .then((result) => { if (!cancelled) setManagedGalleries(result.galleries ?? []); })
      .catch(() => {});
    return () => { cancelled = true; };
  }, []);
  ...
  const uploadFiles = useCallback(async (files: File[]) => {
    ...
    for (const it of items) {
      try {
        ...
        if (isContentSwarmManaged) {
          const categoryName = managedCategoryId
            ? managedGalleries.find((gallery) => gallery.id === managedCategoryId)?.name
            : undefined;
          try {
            await requestContentSwarmMaterials("upload-image", {
              name: it.file.name,
    ...
    if (materialSyncFailures) toast.error(`${materialSyncFailures} 张图片已上传到 HyCanvas，但同步到素材库失败`);
  }, [workspaceId, folderId, managedCategoryId, managedGalleries, refresh, toast]);
  ...
  const createFolder = useCallback(async () => {
    ...
    try {
      ...
      if (isContentSwarmManaged) {
        try {
          const result = await requestContentSwarmMaterials<{ category_id: string }>("ensure-gallery", { name });
          await refreshManagedGalleries();
          setManagedCategoryId(result.category_id);
        } catch {
          toast.error("文件夹已创建，但同步到素材库失败");
        }
      }
      await refreshFolders();
      setFolderId(f.id);
    } catch { toast.error(tr("editor.couldnt_create_that_folder")); }
  }, [workspaceId, refreshFolders, refreshManagedGalleries, toast]);
  ...
  const selectedManagedGallery = managedGalleries.find((gallery) => gallery.id === managedCategoryId) ?? null;
      ...
      <div className="flex flex-col gap-2.5">
        ...
        <CollapsibleSection title={tr("editor.folders")} icon={Folder} defaultOpen>
          ...
          <div className="flex flex-col gap-0.5">
            <button onClick={() => { setFolderId(null); setManagedCategoryId(""); }} className={`flex items-center gap-2 rounded-md px-2 py-1 text-start text-sm ${(isContentSwarmManaged ? managedCategoryId === "" : folderId === null) ? "bg-brand-50 font-medium text-brand-ink" : "text-neutral-600 hover:bg-neutral-100"}`}>
              <Folder size={14} /> {tr("editor.all_uploads")}
            </button>
            {isContentSwarmManaged ? managedGalleries.map((gallery) => (
              <button
                key={gallery.id}
                onClick={() => { setManagedCategoryId(gallery.id); setFolderId(folders.find((folder) => folder.name === gallery.name)?.id ?? null); }}
                className={`flex items-center gap-2 rounded-md px-2 py-1 text-start text-sm ${managedCategoryId === gallery.id ? "bg-brand-50 font-medium text-brand-ink" : "text-neutral-600 hover:bg-neutral-100"}`}
                title={`${gallery.name} · ${gallery.count} 个素材`}
              >
                <Folder size={14} /> <span className="truncate">{gallery.parent_id ? `└ ${gallery.name}` : gallery.name}</span>
              </button>
            )) : folders.map((f) => (
        ...
        <CollapsibleSection title={tr("editor.your_media")} icon={ImagePlus} defaultOpen>
          {isContentSwarmManaged ? (
            <ManagedUploadMedia key={`${managedCategoryId}-${managedRefreshNonce}`} categoryId={managedCategoryId} />
          ) : <>
...
function BuiltInStockPanel({ workspaceId, embedded = false }: { workspaceId: string | null; embedded?: boolean }) {
  const toast = useToast();
  ...
  return embedded ? content : <PanelShell title={tr("editor.stock")}>{content}</PanelShell>;
}

type ContentSwarmMaterialList = { items: ContentSwarmMaterial[]; total: number };
type ContentSwarmGalleryList = { galleries: ContentSwarmGallery[] };
type ContentSwarmMaterialFile = { data_url: string; content_type: string };

const materialPreviewCache = new Map<string, string>();

function ContentSwarmMaterialTile({ item, onPlace }: { item: ContentSwarmMaterial; onPlace: (item: ContentSwarmMaterial) => void }) {
  const [preview, setPreview] = useState(() => materialPreviewCache.get(item.id) ?? "");
  useEffect(() => {
    if (preview) return;
    let cancelled = false;
    void requestContentSwarmMaterials<ContentSwarmMaterialFile>("get-file", { item_id: item.id, purpose: "preview" })
      .then((result) => {
        if (cancelled) return;
        materialPreviewCache.set(item.id, result.data_url);
  ...
  return (
    <button
      ...
      className="group overflow-hidden rounded-lg border border-neutral-200 bg-neutral-50 text-start hover:border-brand-300"
    >
      <span className="grid aspect-square place-items-center overflow-hidden bg-neutral-100">
        {preview ? (
          // eslint-disable-next-line @next/next/no-img-element
          <img src={preview} alt={item.name} className="h-full w-full object-cover transition group-hover:scale-[1.02]" />
        ) : <Spinner />}
      </span>
      <span className="block truncate px-2 py-1.5 text-xs text-neutral-700">{item.name}</span>
    </button>
  );
}

function ManagedUploadMedia({ categoryId }: { categoryId: string }) {
  const toast = useToast();
  const [query, setQuery] = useState("");
  const debouncedQuery = useDebouncedValue(query);
  const requestKey = `${categoryId}\n${debouncedQuery}`;
  const [state, setState] = useState<{ key: string; items: ContentSwarmMaterial[]; error: string }>({ key: "", items: [], error: "" });
  ...
  useEffect(() => {
    ...
    void requestContentSwarmMaterials<ContentSwarmMaterialList>("list-items", {
      ...
      if (!cancelled) setState({ key: requestKey, items: result.items ?? [], error: "" });
    }).catch((cause) => {
      if (!cancelled) setState({ key: requestKey, items: [], error: cause instanceof Error ? cause.message : "素材读取失败" });
    });
    return () => { cancelled = true; };
  }, [categoryId, debouncedQuery, requestKey]);

  async function place(item: ContentSwarmMaterial) {
    if (placing) return;
    setPlacing(item.id);
    try {
      const file = await requestContentSwarmMaterials<ContentSwarmMaterialFile>("get-file", { item_id: item.id, purpose: "insert" });
      placeImage(file.data_url, { origin: "contentswarm-material", materialItemId: item.id, categoryId: item.category });
    } catch (cause) {
      toast.toast(cause instanceof Error ? cause.message : "素材插入失败", "error");
    } finally {
      setPlacing(null);
    }
  }
  ...
  return (
    <>
      <div className="relative">
        <Search size={14} className="pointer-events-none absolute start-2.5 top-1/2 -translate-y-1/2 text-neutral-400" />
        <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索素材库" className="w-full rounded-lg border border-neutral-200 py-1.5 ps-8 pe-8 text-sm outline-none focus:border-brand-400" />
        {query && <button onClick={() => setQuery("")} title="清除搜索" className="absolute end-2 top-1/2 grid h-5 w-5 -translate-y-1/2 place-items-center rounded text-neutral-400 hover:text-neutral-700"><X size={13} /></button>}
      </div>
      {loading ? (
        <div className="grid place-items-center py-8 text-neutral-400"><Spinner /></div>
      ) : state.error ? (
        <p className="py-6 text-center text-xs text-red-500">{state.error}</p>
      ) : state.items.length === 0 ? (
        <p className="py-6 text-center text-xs text-neutral-400">当前分类暂无素材</p>
      ) : (
        <div className={`grid grid-cols-2 gap-2 ${placing ? "pointer-events-none opacity-70" : ""}`}>
          {state.items.map((item) => <ContentSwarmMaterialTile key={item.id} item={item} onPlace={(selected) => void place(selected)} />)}
        </div>
      )}
    </>
  );
...
function ContentSwarmMaterialPanel() {
  ...
  useEffect(() => {
    let cancelled = false;
    void requestContentSwarmMaterials<ContentSwarmGalleryList>("list-galleries")
      .then((result) => { if (!cancelled) setGalleries(result.galleries ?? []); })
      .catch((cause) => { if (!cancelled) setGalleryError(cause instanceof Error ? cause.message : "素材分类读取失败"); });
    return () => { cancelled = true; };
  }, []);
  ...
  useEffect(() => {
    ...
    void requestContentSwarmMaterials<ContentSwarmMaterialList>("list-items", {
      ...
      if (!cancelled) setMaterialState({ key: requestKey, items: result.items ?? [], error: "" });
    }).catch((cause) => {
  ...
  return (
    <PanelShell title="素材库">
      ...
      {source === "system" ? <CoverElementsPanel /> : <>
      ...
      <div className="mb-3 flex max-h-24 flex-wrap gap-1 overflow-y-auto">
        ...
        {orderedGalleries.map((gallery) => (
          <button
            ...
            className={stockChipCls(activeCategory === gallery.id)}
            title={`${gallery.name} · ${gallery.count} 个素材`}
          >
            {gallery.parent_id ? `└ ${gallery.name}` : gallery.name}
          </button>
        ))}
      </div>
      {loading ? (
        ...
        <p className="mt-6 text-center text-xs text-red-500">{error}</p>
      ) : items.length === 0 ? (
        <p className="mt-6 text-center text-xs text-neutral-400">当前分类暂无素材</p>
      ) : (
        <div className={`grid grid-cols-2 gap-2 ${placing ? "pointer-events-none opacity-70" : ""}`}>
          {items.map((item) => <ContentSwarmMaterialTile key={item.id} item={item} onPlace={(selected) => void place(selected)} />)}
        </div>
      )}
      </>}
    </PanelShell>
