...
export function DashboardApp({ view }: { view: DashboardView }) {
  ...
  async function applyTemplate(template: TemplateSummary | string) {
    ...
    try {
      ...
      const { designId } = await oc.applyTemplate(templateId, activeWorkspaceId);
      await open(designId);
    } catch {
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-neutral-50 text-neutral-900" onClick={() => setMenuFor(null)}>
      ...
      <main className="oc-scroll flex-1 overflow-y-auto" style={DASH_BACKDROP}>
        ...
        <div className="sticky top-0 z-30 flex items-center gap-3 border-b border-neutral-200 bg-surface/95 px-6 py-2.5 backdrop-blur">
          {isContentSwarmManaged && (
            <nav className="flex shrink-0 items-center gap-1 border-e border-neutral-200 pe-3 text-sm" aria-label="设计工作台导航">
              <ManagedNavItem icon={HomeIcon} label="设计首页" active={view === "home"} onClick={() => gotoView("home")} />
              <ManagedNavItem icon={LayoutTemplate} label="模板" active={view === "templates"} onClick={() => gotoView("templates")} />
              <ManagedNavItem icon={Star} label="收藏" active={view === "favorites"} onClick={() => gotoView("favorites")} />
              <ManagedNavItem icon={CheckSquare} label="任务" active={view === "tasks"} onClick={() => gotoView("tasks")} />
            </nav>
          )}
          ...
          <div className="ms-auto flex items-center gap-2">
            ...
            />
            {!isContentSwarmManaged && <NotificationsBell />}
            {!isContentSwarmManaged && <UserMenu user={user} onSettings={() => void router.push("/settings")} onSignOut={() => void logout()} />}
          </div>
        </div>
        <div className="mx-auto max-w-[1600px] px-6 py-8">
          {view === "home" && (
          <>
          {!isContentSwarmManaged && <VerifyEmailBanner />}
          ...
          {view === "templates" && (
            <section>
              ...
              {templateZone && (
                <div className="mb-4 flex items-center gap-2 rounded-xl border border-brand-200 bg-brand-50 px-3 py-2 text-sm text-brand-ink">
                  ...
                  <button
                    ...
                    className="rounded-lg px-2.5 py-1 text-xs font-medium hover:bg-brand-100"
                  >
                    {tr("dashboard.view_all_templates")}
                  </button>
                </div>
              )}
              ...
              {filteredTemplates.length === 0 && zoneDesigns.length === 0 && !templateZone ? (
...
function ManagedNavItem({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className={`flex h-8 items-center gap-1.5 rounded-lg px-2.5 font-medium transition ${
        active ? "bg-brand-50 text-brand-ink" : "text-neutral-600 hover:bg-neutral-100 hover:text-neutral-800"
      }`}
    >
      <Icon size={15} />
      <span>{label}</span>
