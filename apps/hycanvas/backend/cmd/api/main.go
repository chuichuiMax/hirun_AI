...
func main() {
	...
	if cfg.AuthMode == "contentswarm" {
		authPolicy = config.AuthPolicy{}
		logger.Info("ContentSwarm managed auth enabled", "contentSwarmURL", cfg.ContentSwarmURL)
	} else if !authPolicy.PasswordLogin && !authPolicy.MagicLinkLogin && !authPolicy.OidcLogin {
	...
	srv := &http.Server{
		...
		Handler: httpapi.NewRouter(httpapi.Deps{
			...
			Auth:            authPolicy,
			ManagedAuth:     cfg.AuthMode == "contentswarm",
			ContentSwarmURL: cfg.ContentSwarmURL,
