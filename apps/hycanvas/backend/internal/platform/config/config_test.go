...
func TestLoadContentSwarmManagedAuth(t *testing.T) {
	t.Setenv("DATABASE_URL", "postgres://x")
	t.Setenv("HYCANVAS_AUTH_MODE", "contentswarm")
	t.Setenv("CONTENTSWARM_URL", "http://127.0.0.1:5173/")

	c, err := Load()
	if err != nil {
		t.Fatalf("Load: %v", err)
	}
	if c.AuthMode != "contentswarm" || c.ContentSwarmURL != "http://127.0.0.1:5173" {
		t.Fatalf("unexpected managed auth config: mode=%q url=%q", c.AuthMode, c.ContentSwarmURL)
	}
}

func TestLoadContentSwarmManagedAuthRequiresURL(t *testing.T) {
	t.Setenv("DATABASE_URL", "postgres://x")
	t.Setenv("HYCANVAS_AUTH_MODE", "contentswarm")
	t.Setenv("CONTENTSWARM_URL", "")

	if _, err := Load(); err == nil {
		t.Fatal("managed auth without CONTENTSWARM_URL should fail")
	}
}
