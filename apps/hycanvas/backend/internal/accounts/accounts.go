...
const IntegrationTicketTTL = time.Minute
...
// IssueIntegrationTicket creates a short-lived credential for a trusted
// workspace integration. The caller must already have verified the API key and
// design tenancy; the browser exchanges this ticket for a normal session.
func (s *Service) IssueIntegrationTicket(userID, designID string) (string, error) {
	if strings.TrimSpace(userID) == "" || strings.TrimSpace(designID) == "" {
		return "", ErrToken
	}
	return jwt.SignAudience(userID, "contentswarm-editor:"+designID, s.jwtSecret, IntegrationTicketTTL)
}

func (s *Service) IssueWorkspaceIntegrationTicket(userID, workspaceID string) (string, error) {
	if strings.TrimSpace(userID) == "" || strings.TrimSpace(workspaceID) == "" {
		return "", ErrToken
	}
	return jwt.SignAudience(userID, "contentswarm-workspace:"+workspaceID, s.jwtSecret, IntegrationTicketTTL)
}

// RedeemIntegrationTicket exchanges a trusted integration credential for the
// same server-tracked session used by the regular editor.
func (s *Service) RedeemIntegrationTicket(ctx context.Context, token, designID, device, ip string) (*Tokens, error) {
	claims, err := jwt.Verify(token, s.jwtSecret)
	if err != nil || claims.Aud != "contentswarm-editor:"+designID {
		return nil, ErrToken
	}
	u, err := s.findUserByID(ctx, claims.Sub)
	if err != nil {
		return nil, ErrToken
	}
	_, tokens, err := s.issueSession(ctx, u, device, ip)
	return tokens, err
}

func (s *Service) RedeemWorkspaceIntegrationTicket(ctx context.Context, token, workspaceID, device, ip string) (*Tokens, error) {
	claims, err := jwt.Verify(token, s.jwtSecret)
	if err != nil || claims.Aud != "contentswarm-workspace:"+workspaceID {
		return nil, ErrToken
	}
	u, err := s.findUserByID(ctx, claims.Sub)
	if err != nil {
