...
func TestSeedAndSearch(t *testing.T) {
	...
	for _, a := range photos {
		...
		for _, field := range []string{"previewUrl", "sourceUrl"} {
			if !strings.HasPrefix(str(a[field]), "data:image/svg+xml;base64,") {
				t.Fatalf("seed photo %q is not bundled locally: %s=%q", a["id"], field, a[field])
			}
		}
	}
...
func TestProxyServesLegacyPicsumPhotoLocally(t *testing.T) {
	s := &Service{}
	data, mime, err := s.Proxy("https://picsum.photos/id/1080/1600/1200")
	if err != nil {
		t.Fatal(err)
	}
	if mime != "image/svg+xml" || !strings.Contains(string(data), "<svg") {
		t.Fatalf("unexpected local legacy response: mime=%q bytes=%d", mime, len(data))
	}
}
