...
func TestAPIKeySurface_DB(t *testing.T) {
	...
	r.Route("/api/v1", func(api chi.Router) {
		mountAuth(api, acct, false, config.AuthPolicy{}, nil)
	...
	// A trusted integration can mint a design-bound browser ticket without
	// exposing the API key to the browser.
	res = do("POST", "/api/v1/auth/integration-ticket/design-in-ws", minted.Key, nil)
	if res.StatusCode != http.StatusCreated {
		t.Fatalf("integration ticket: want 201, got %d", res.StatusCode)
	}
	var ticket struct {
		Ticket string `json:"ticket"`
	}
	_ = json.NewDecoder(res.Body).Decode(&ticket)
	res.Body.Close()
	if ticket.Ticket == "" {
		t.Fatal("integration ticket is empty")
	}
	client := &http.Client{CheckRedirect: func(_ *http.Request, _ []*http.Request) error { return http.ErrUseLastResponse }}
	redeemURL := srv.URL + "/api/v1/auth/integration?ticket=" + ticket.Ticket + "&designId=design-in-ws&next=%2Feditor%2F%3Fid%3Ddesign-in-ws"
	res, err = client.Get(redeemURL)
	if err != nil {
		t.Fatalf("redeem integration ticket: %v", err)
	}
	if res.StatusCode != http.StatusFound || res.Header.Get("Location") != "/editor/?id=design-in-ws" {
		t.Fatalf("redeem integration ticket: status=%d location=%q", res.StatusCode, res.Header.Get("Location"))
	}
	if len(res.Cookies()) != 2 {
		t.Fatalf("redeem integration ticket should set two auth cookies, got %d", len(res.Cookies()))
	}

	// The ContentSwarm menu uses a workspace-bound ticket to embed the HyCanvas
	// dashboard without granting a reusable credential to the browser.
	res = do("POST", "/api/v1/auth/integration-ticket/workspace/"+ws.ID, minted.Key, nil)
	if res.StatusCode != http.StatusCreated {
		t.Fatalf("workspace integration ticket: want 201, got %d", res.StatusCode)
	}
	_ = json.NewDecoder(res.Body).Decode(&ticket)
	res.Body.Close()
	redeemURL = srv.URL + "/api/v1/auth/integration?ticket=" + ticket.Ticket + "&workspaceId=" + ws.ID + "&next=%2Fdashboard%2F"
	res, err = client.Get(redeemURL)
	if err != nil {
		t.Fatalf("redeem workspace integration ticket: %v", err)
	}
	if res.StatusCode != http.StatusFound || res.Header.Get("Location") != "/dashboard/" {
		t.Fatalf("redeem workspace integration ticket: status=%d location=%q", res.StatusCode, res.Header.Get("Location"))
	}
	res.Body.Close()
