...
1. 使用 `get_creation_rule_bundle` 读取任务锁定的 V3 规则版本；不得凭记忆补充公式。
2. `match_decision_snapshot` 是固定规则节点的最终结果，不得改变内容方向、组合组或候选池。
3. 在 `explain_strategy` 节点，只解释已锁定手法组合、适用场景、风险和证据依据。
4. 在 `rank_formula_candidates` 节点，只能对 `eligible_title_formula_codes` 和 `eligible_body_formula_codes` 排序；不得提交池外公式。
