...
3. 对照 `strategy_snapshot` 检查创作手法、标题公式和正文结构，对照 ContentBrief 与 EvidenceBundle 检查事实、人设、语气和来源。
   ...
   - 若 `content_brief.form_values.mp_service_entry` 为「装修家居」：标题若呈「楼盘/面积/风格」说明书式平铺、缺少情绪/悬念/反差/利益吸引点，以 `TITLE_FACT_UNSUPPORTED` 或 `PERSONA_STYLE_MISMATCH` 阻断并要求改出钩子；正文若展开某套房旧况改造完工案例故事，以 `FACT_CHECK_FAILED` 阻断并要求改为信息卡点；非工艺展示时，正文应能看到简报已有的小区、面积、风格与「鸿扬家装」施工归属，以及品牌优势与引流点；收尾须📍分行＋▫️列表＋💬评论，若把城市/小区/面积揉成「在某地朋友，如果也在为某盘某㎡」一句墙字，以 `PERSONA_STYLE_MISMATCH` 阻断；布局仅在简报有填时要求出现，不得因缺失布局字段而编造后通过。若标题、正文、话题或封面文案把鸿扬写成「整装」「标准化整装」或「全屋整装」，以 `PERSONA_STYLE_MISMATCH` 阻断并要求改为「定制化家装」。标题、正文、话题或封面若出现「合同价」，以 `PERSONA_STYLE_MISMATCH` 阻断并要求改为「预算价」（数字保持原样，不算改事实）。标题、正文、话题或封面若出现「口径」，以 `PERSONA_STYLE_MISMATCH` 阻断并改为「钱花在哪／费用怎么拆／预算清不清楚」。项目阶段为泥木阶段时，成品写「泥木」或以泥木带过未出现的木工，以 `PERSONA_STYLE_MISMATCH` 或 `FACT_CHECK_FAILED` 阻断并要求改为「泥瓦」；仅当工艺类型/工艺名称或证据明确出现木工时才允许写木工。若内容类型为「装修报价清单」/「报价清单」：标题语义不清、词库硬拼看不懂时阻断；标题正例为「142㎡旧房翻新，钱要花在哪？」这类口语句；标题或正文若出现面积区间（如130-150㎡、130-150m）而简报已锁定具体㎡，以 `FACT_CHECK_FAILED` 阻断并改为锁定㎡；正文若以低价/更便宜为主卖点而弱化鸿扬品牌优势，以 `PERSONA_STYLE_MISMATCH` 阻断并要求改为品牌与交付优势收束。若内容类型为「工艺施工展示」/「工艺展示」：标题未点明工艺/阶段主题、或把「听劝/真香/远超预期」等词库词硬拼在工艺名后（如「水电施工听劝，规范验收远超预期」）时阻断；正文若写成案例分享（旧况→改造→完工）而弱化工艺类型/工艺名称/项目阶段与标准讲解，或以装修风格、小区面积资料卡为主线，以 `FACT_CHECK_FAILED` 或 `PERSONA_STYLE_MISMATCH` 阻断并要求改为工艺讲解（定制化家装＋鸿扬家装＋工艺名称＋工艺类型＋关键环节）；不得因缺少风格、小区或面积而阻断。标题或正文强调装修风格时同样阻断。
