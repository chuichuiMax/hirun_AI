...
5. 先读取 `payload.product_evidence_pack.slot_mappings`，把 `target_usage=title` 的产品资料映射到标题公式槽位；只使用 EvidenceBundle 中 `allowed_usage` 包含 `title` 的事实。
6. `style_reference` 证据只能借鉴结构和节奏，禁止复制原句、事实或数字，也不得放入标题的 `evidence_ids`。
7. 生成 3～5 个差异明确的候选，保持同一创作手法和同一锁定公式。
8. 严格提交 `TitleCandidatesResultV1`：顶层只包含 `candidates`、`selected_title_formula_code`、`evidence_ids`；每个候选只包含 `id`、`text`、`formula_code`、`evidence_ids`、`reason`。
9. 数字、价格、参数和效果必须关联证据 ID；证据不足时不生成该表达。
