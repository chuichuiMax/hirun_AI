...
1. 只读取当前节点 `payload`；在简化工作流中与标题、大纲一次生成并保持一致。
   ...
   - 若入口为「装修家居」：遵守 `writing_instruction`；**不要**展开某套房的案例故事（旧况→改造过程→完工效果）；信息卡点优先写出小区名称、房屋面积、房屋布局（仅简报有填时）、风格、项目施工鸿扬家装；必须写鸿扬家居/鸿扬家装品牌优势（**定制化家装**，禁止「整装」「标准化整装」），并带明确引流点（同城咨询、留言、评论区）；品牌或案例证明只作一句背书，不得编造户型缺陷与改造前后细节。
