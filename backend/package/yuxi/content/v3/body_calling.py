...
DECORATION_BODY_CALLING: dict[str, dict[str, Any]] = {
    "C01": {
        ...
        "sections": [
            {
                "id": "audience_pain",
                "name": "人群痛点开篇",
                "instruction": "锁定本地装修业主，聚焦看不懂费用明细、怕隐形增项、怕低价陷阱等困扰；痛点可谈预算，但不要把「低价」塑造成解决方案，成品禁用平台封禁词（如「报价」「私信」）",
                "fill_rule": "仅使用预算痛点词库描述业主普遍预算困扰；词库含封禁问题词时必须改写后再写入正文",
            ...
            },
            {
                ...
                "fill_rule": "只使用 ContentBrief 或 EvidenceBundle 中的真实数据，逐字复制匹配条目的 id，不调用词库补造数字，不编造布局与改造细节，面积只抄锁定的具体㎡禁止写区间",
                "lexicon_calls": [],
        ...
        "reference_examples": [
            "从看不懂费用明细、怕增项切入；用小区、面积、风格与参考费用卡点说明；重点讲鸿扬品牌与透明交付，最后邀请同城咨询或留言。"
        ],
    },
    "C02": {
        ...
        "sections": [
            ...
            },
            {
                ...
                "fill_rule": "只使用 ContentBrief 或 EvidenceBundle 中的真实字段，逐字复制匹配条目的 id；缺失布局则跳过该卡点；面积只抄锁定的具体㎡禁止写区间",
                "lexicon_calls": [],
                "fact_source": "evidence",
            },
            {
    ...
    "C03": {
        ...
        "sections": [
            ...
            },
            {
                ...
                "name": "业主误区罗列",
                "instruction": "呈现与当前工艺相关的新手高频错误做法和认知误区",
                "fill_rule": "使用装修误区词库，并与当前工艺主题保持一致",
        ...
        "variants": [],
        "variation_rule": "悬念、误区和忠告表达应轮换，避免多篇正文话术重复；工艺展示类始终以工艺讲解为主线",
        "reference_examples": [
            "以内行工艺细节制造悬念，指出新手误区，给出有依据的标准做法并点名具体工艺，最后用真诚匠人忠告收尾。"
        ],
    },
    "C04": {
