...
- 内容工作流升级为 V3.4：标题候选通过确定性校验后，由标题 Agent 从 `selectable=true` 的候选中自动选择最终标题，不再触发人工标题选择；Agent 只能提交候选 ID 和理由，禁止改写标题文本、公式或 Evidence ID。现阶段从工作流移除视觉规划、封面生成、外部等待、视觉审核、人工选封面及发布打包节点，内容通过最终人工审批后直接保存无封面也合法的内容资产版本；前端同步移除“视觉与交付”分组。V3.3 历史 checkpoint 保持只读，新任务默认使用 V3.4。
