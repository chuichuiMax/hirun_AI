...
- 根目录 Docker Compose 纳入 ContentSwarm 托管模式的 HyCanvas 应用、独立 PostgreSQL 与幂等初始化任务；本地启动会自动创建集成用户、固定工作区和具备生成/读取/导出权限的 API Key，并向 API/Worker 注入服务地址、浏览器地址、Key 与工作区配置，使新环境拉取代码后无需手工配置即可使用视觉创作。生产环境仍须覆盖默认开发密钥。
...
- 好评笔记与装修家居生产分流：内容工作台阶段 1 仅保留「装修与家居」和「好评笔记」两个入口，不再展示其他行业卡片。创建任务后的素材输入框读取变量配置中对应服务入口、PC 端口与当前版本（简化版/专业版）的启用变量，不再使用行业模板静态字段。好评笔记是业主第一人称评价设计师、预算师、项目经理、客户经理等项目成员，简报要求模仿「好评知识库」已有文章，内容目标只用品牌/教育、禁止获客转化；审批通过后跳过封面规划、提交、等待、视觉审核和人工选封面，直接保存无封面资产。装修家居仍走获客链路和封面等待。小程序好评笔记提交后直接开跑，失败自动重试且不展示失败页，最终审批自动通过。所在区域可选地级市或区县，例如「长沙市」或「长沙市芙蓉区」。
