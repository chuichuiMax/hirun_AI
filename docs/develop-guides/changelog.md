...
- HyCanvas 可编辑封面模板新增一期结构化内容填充：模板作者在另存模板时可从文字图层声明可填字段、语义角色、稳定编码、必填和最大字数；ContentSwarm 托管编辑器中首次保存尚未配置字段的设计，会在设计快照保存成功后直接弹出模板字段设置，字段保存后写回设计元数据，后续普通保存不再重复弹窗。字段声明随模板设计快照保存并由模板目录返回。内容封面任务按 AI 已生成的锁定视觉文案和事实简报精确映射标题、项目名称、面积、设计师、完成年份及品牌等字段，不再只按“标题/副标题”名称猜测；实例化前后端共同拦截缺少必填事实和超长文案，生成后的 HyCanvas 文字仍保持原位置、样式和可编辑能力。一期不包含图片 OCR 和自动拆层。
