...
- 内容工作流右侧新增实时执行轨迹，以中文内容卡片流式展示 Agent 提交的决策依据与产出、节点输入与输出、Skill、工具、规则校验和知识库命中片段，并在任务完成、失败或刷新恢复后保留可审计记录；不展示模型隐藏思维链或 JSON 代码，事件预览统一限制字段、条数和长度并过滤敏感配置。结果页不再展示标题、正文公式内部编码，改为呈现公式名称、目标说明和正文结构。
