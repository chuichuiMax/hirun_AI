...
- 好评笔记与装修家居生产分流：内容工作台阶段 1 仅保留「装修与家居」和「好评笔记」两个入口，不再展示其他行业卡片。创建任务后的素材输入框读取变量配置中对应服务入口、PC 端口与当前版本（简化版/专业版）的启用变量，不再使用行业模板静态字段。装修家居沿用原来的获客 V3 生产：加载装修标题/正文资料库、有证据缺口时走调研 Agent、人工开跑与审批，并继续走封面规划、生成与选择。好评笔记按业主第一人称写口碑文案，简报要求模仿「好评知识库」已有文章，内容目标只用品牌/教育、禁止获客转化；不加载装修公式词库、不走获客调研 Agent、不把装修获客标题/正文公式写入锁定策略，语义审核也不按 T01/C04 等获客公式阻断，只审第一人称、项目成员评价、事实和风险；审批通过后跳过封面规划、提交、等待、视觉审核和人工选封面，直接保存无封面资产。PC 好评笔记不要求图库原图和 HyCanvas 模板，填完后点「生成内容」即编译简报并启动工作流；装修家居仍需选择图库原图与模板，编译后点「开始生成」。小程序好评笔记提交后直接开跑，失败自动重试且不先展示失败页，最终审批自动通过。所在区域可选地级市或区县，例如「长沙市」或「长沙市芙蓉区」。
