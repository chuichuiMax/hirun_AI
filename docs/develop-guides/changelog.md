...
- 封面生成新增“大字报”模式：在模板复刻右侧提供账号隔离的蒙版素材库，支持最多 100 张 PNG/JPG/WebP 批量拖拽导入、分类、标签检索、状态筛选、分页、重新分析、禁用和删除；透明蒙版自动识别产品区域，不透明画板需人工标注后启用。用户可上传产品图并调整 contain/cover、缩放、焦点和位移，后端以 1080×1440 确定性图层合成保留模板文字、Logo 和装饰；关联内容资产时仅概括改写可编辑文字槽。可选 image2 只美化未锁定的背景和融合区域，生成后重新叠加产品主体与模板上层，默认最高质量 PNG。任务接入现有队列、SSE、重试、历史恢复、下载和当前封面流程，并增加尺寸、透明度、溢出和锁定策略质量门禁、60 组 Golden Image、Worker 与真实 API 闭环测试。蒙版软删除后可重新导入同一素材，批量冲突通过事务保存点逐项隔离。受保护的蒙版缩略图和历史产品图改为通过鉴权 API 下载 Blob 后显示，不再因原生 `<img>` 缺少 Bearer Token 返回 401；刷新、删除和页面退出时会释放临时 URL。大字报前端进一步重构为“选择蒙版、上传产品、调整内容、预览生成”四步工作台：导入区域按需展开，筛选空状态可一键恢复且不会丢失当前创作上下文，不透明蒙版可在画布上实时查看产品替换框，参数支持一键恢复默认，image2 设置折叠为可选高级项，最终操作区集中显示蒙版、产品图和预览就绪状态及不可生成原因。
...
- 新增内容发布账号、员工与封面管理：侧栏「账号管理」「员工管理」「封面管理」「配置管理」与「智能体管理」同级；账号支持名称、ID、企业号/个人号、关注数、粉丝数、获赞与收藏、作品数、启停状态的查询与增删改，数据分析入口展示当前统计字段；员工支持员工编码、姓名、登录账号、性别、登录端口、角色、启停状态的查询与增删改，不含手机号码字段；封面按中国风骨/欧洲经典/现代之源分类展示，支持新增、启停和删除，并展示图片名、生成量与创建时间；同一风格下图片名不可重复，支持按图片名模糊查询。开发环境 Vite 增加 `/public` 到 MinIO 的代理，封面等已上传图片可正常展示。「配置管理」为可展开目录，其人设管理、权限配置、内容类型配置、变量配置为配置管理的子页面。权限配置维护角色编码/名称，系统用户角色（普通用户/管理员/超级管理员）与内容角色累加展示并联动：关联人员同时统计员工与系统用户，用户管理的角色筛选与分配来自同一角色表，角色改名同步员工与用户，系统角色不可删除。员工新增/编辑时的角色下拉来自该角色表；关联人员弹窗按姓名、编码模糊查询；「授权」进入角色权限矩阵，按模块勾选能力并保存。内容类型配置维护 NRLX 编码与类型名称，支持按编码/名称模糊查询、启停、编辑和删除。变量配置维护 FWTD 编码、变量名称与服务入口，服务入口为装修家居、好评笔记；默认预置好评笔记 5 条与装修家居 4 条，支持按编码/变量名称/服务入口模糊查询、启停、编辑和删除。新增员工端获客小程序 `/api/mp/*`：短信/微信登录（微信必须授权手机号，并校验员工管理中该登录账号存在且已启用、开通 app 端口；点「获取验证码」即校验手机号与员工账号，未登记、停用或未开通 app 时不发短信；开发环境短信返回 `debug_code`，生产未配置短信或微信时返回 503）、动态表单、编译事实简报后锁定、quick 模式开跑 V3 工作流、内容列表收藏/复制/删除，以及「我的」简介；小程序只使用该前缀，不复用 PC 内容与密码登录接口。内容管理只列出已生成内容编号的任务，无标题时不展示「未生成标题」占位。装修家居「所在区域」为湖南 14 个地级市，选定地级市后再选区县（如株洲市荷塘区、云龙示范区）。外框面积按 8.24 报价表带出基础、木制品、主材价格范围，并在区间内按「下限、下限+0.5、之后取整」自动取值（如 15-18万 取 15 / 15.5 / 16 / 17 / 18）。小程序首页按获客门户原型实现增长指标、能力轮播与四步链路，入口进入内容生成。登录页按原型为「密码登录」：手机号+验证码、红色登录、绿色一键登录；一键登录进入微信授权页（昵称头像，再授权手机号，须与员工管理账号一致），授权后进入登录确认页确认或取消。
