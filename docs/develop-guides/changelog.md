...
- 强化生产部署安全门禁：部署前强制校验 JWT、实例标识、PostgreSQL、Neo4j、MinIO 与小红书网关凭据，拒绝缺失、公开默认值或长度不足的配置；要求运营入口使用可验证的 HTTPS 地址，并在部署后同时验证本机 API、内部浏览器网关和公网 TLS 入口。发布 CI 将后端门禁按能力域拆为独立 pytest 进程并补充封面、OCR 集成测试，降低单进程依赖导入导致的内存峰值而不减少覆盖；API、Web 与 Sandbox Provisioner 全部候选镜像成功后才统一提升正式标签，防止部分发布，并从正式标签解析最终 digest 归档。生产部署必须提供该 digest 清单，Compose 直接运行 `仓库@sha256:...`，成功清单随版本归档供精确回滚；仅历史版本允许显式启用标签兼容回滚。一键部署会在上传任何文件前先流式执行远程只读资源预检，并在正式启动前复核同一门禁。
