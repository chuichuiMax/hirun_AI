...
- 内容生产工作流升级为 V3.2：保留公式锁定前的通用证据检索，并在唯一标题/正文公式锁定后新增策略驱动的产品资料二次 RAG。固定节点从完整 `StrategySnapshot` 解析产品介绍、价格、案例证明、品牌与爆款结构参考需求，内容调研 Agent 使用独立 `strategy-product-researcher` Skill 在“智能体管理”中配置且当前用户有权访问的知识库内定向检索，内容任务表单不再重复选择知识库；检索生成带来源、风险和公式槽位映射的产品证据，价格、优惠与效果承诺必须人工确认后才能写入新版不可变 EvidenceBundle。标题、大纲、正文和润色节点只消费绑定策略 hash 与证据 hash 的 `ProductEvidencePack`，仍禁止直接检索知识库；确定性校验会阻断未实际引用映射产品资料的候选或正文。爆款样例仅允许 `style_reference/structure_reference_only`，不能作为事实、引用或数字白名单来源。新工作流使用独立版本 ID，历史 V3/V3.1 checkpoint 保持只读。内容方向与关键事实确认界面把内部 Evidence ID 转换为“中文字段名称：证据值”，保留内部追踪 ID 但不再直接向运营人员展示。产品资料契约进一步绑定槽位、目标用途和资料类型，阻止案例证据冒充爆款结构参考；结构化结果工具只锁定一次有效提交，校验失败会把明确原因返回 Agent 并允许其在节点预算内修正重提，避免单次错误映射直接终止工作流。
