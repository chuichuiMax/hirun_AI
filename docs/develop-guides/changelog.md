...
- 新增账号隔离的“素材库”一级入口，提供素材图片与封面模板两个模块；复用 `ContentCoverAsset` 文件事实并新增素材库目录项，所有新素材统一写入私有 `image` 桶的用户隔离路径。封面模板继续复用大字报分析和生成链路；提供只复制不删除旧对象的 dry-run/apply/verify/rollback 迁移脚本，将 `source`、`template`、`mask`、`poster_template` 安全迁入 `image` 桶，生成结果等非素材对象保持原位。
